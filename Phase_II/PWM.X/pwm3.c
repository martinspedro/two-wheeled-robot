/** 
 * \file   pwm3.c
 * \brief  Pulse Width Modulation Module 3 Configuration C file
 * \pre    Uses Timer 2
 * 
 * \author Pedro Martins
 * \date   Created on May 6, 2018, 9:29 AM
 */
...
void output_compare_3_init(void) 
{
    ...
    DISABLE_OUTPUT_COMPARE_PERIPHERAL_3;     // Disable PWM output pin while doing the setup now
...
uint8_t set_PWM3_duty_cycle(uint8_t duty_cycle) 
{
    ...
    #ifdef PWM_ROUNDING
        new_OC3RS  = ( ( (PR2 * duty_cycle) * 10 / 100) + 5) / 10;
    #else
        new_OC3RS = (PR2 * duty_cycle) / 100;
    #endif 

    OC3RS = (uint16_t) (new_OC3RS & 0x0000FFFF);
