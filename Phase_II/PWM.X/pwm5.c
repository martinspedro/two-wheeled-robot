/** 
 * \file   pwm5.c
 * \brief  Pulse Width Modulation Module 5 Configuration C file
 * 
...
void output_compare_5_init(void) 
{
    ...
    DISABLE_OUTPUT_COMPARE_PERIPHERAL_5;     // Disable PWM output pin while doing the setup now
...
uint8_t set_PWM5_duty_cycle(uint8_t duty_cycle) 
{
    ...
    if (duty_cycle > 100)
    {
        ...
        new_OC5RS  = ( ( (PR2 * duty_cycle) * 10 / 100) + 5) / 10;
    #else
        new_OC5RS = (PR2 * duty_cycle) / 100;
    #endif 

    OC5RS = (uint16_t) (new_OC5RS & 0x0000FFFF);
