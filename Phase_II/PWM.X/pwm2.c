/** 
 * \file   pwm2.h
 * \brief  Pulse Width Modulation Module 2 Configuration C file
 * \pre    Uses Timer 2
 * 
...
void output_compare_2_init(void) 
{
    ...
    DISABLE_OUTPUT_COMPARE_PERIPHERAL_2;     // Disable PWM output pin while doing the setup now
...
uint8_t set_PWM2_duty_cycle(uint8_t duty_cycle) 
{
    ...
    #ifdef PWM_ROUNDING
        new_OC2RS  = ( ( (PR2 * duty_cycle) * 10 / 100) + 5) / 10;
    #else
        new_OC2RS = (PR2 * duty_cycle) / 100;
    #endif 

    OC2RS = (uint16_t) (new_OC2RS & 0x0000FFFF);
